...
for(const [sub,items] of Object.entries(PBall)){if(!Array.isArray(items))continue;for(const q of items){const mSig=q.media?(q.media.src||(q.media.svg?'svg'+String(q.media.svg).length+String(q.media.svg).slice(0,80):'')):'';const base=normQ([q.q,...(q.left||[]),...(q.items||[]),...(q.opts||[]),q.passage||''].join(' '));const k=base?base+(mSig?'|'+mSig:''):'';if(k){if(seenQ.has(k))problems.push(`Повтор формулировки: ${sub}/${q.id} = ${seenQ.get(k)}`);else seenQ.set(k,sub+'/'+q.id)}
